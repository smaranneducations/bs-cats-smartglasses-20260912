...
```mermaid
flowchart LR
    A[Define a domain] --> B[Build concepts and ontology]
    B --> C[Collect evidence and data]
    C --> D[Curate governed objects]
    D --> E[Create analysis and stories]
    E --> F[Render reusable media]
    F --> G[Human approves exact artifact]
    G --> H[Publish through channel adapters]
    H --> I[Measure attention, intent, cost and revenue]
    I --> J[Propose evaluated improvements]
    J --> B
```
...
```mermaid
flowchart TB
    C[Ethical constitution and financial authority]
    P[Project charter and business strategy]
    D[Domain profile and source policy]
    O[Ontology, concepts and constraints]
    K[Versioned objects, evidence and lineage]
    X[Context engine and semantic tools]
    H[Typed task harness and bounded agents]
    R[Research, creative and release workflows]
    V[Audience and operator views]
    M[Metrics, feedback and evaluations]

    C --> P --> D --> O --> K --> X --> H --> R --> V --> M
    M -. evaluated change .-> O
    M -. evaluated change .-> H
    K -. correction impact .-> R
```

...
async function request(url,options={}){const response=await fetch(url,options);if(!response.ok){let message=`Request failed (${response.status})`;try{const body=await response.json();message=typeof body.detail==='string'?body.detail:JSON.stringify(body.detail||body);}catch{}throw new Error(message)}return response.status===204?{}:response.json()}
...
(() => {
  ...
  function stageWorkspaceMarkup(stage) {
    ...
    if (!catalog.ready) return `<section class="stage-data"><div class="data-heading"><div><p class="kicker">DATA &amp; SCHEMA</p><h3>Stage workspace</h3></div></div><p class="note">${safe(catalog.error || 'Loading governed records...')}</p></section>`;
  ...
  function renderRecord(id) {
    ...
    body.querySelector('#stageObjectForm').onsubmit = async event => {
      ...
      try {
        ...
        await request('/v1/objects/' + encodeURIComponent(item.object_id) + '/curate', { method: 'POST', headers: { 'Idempotency-Key': 'operator-edit-' + crypto.randomUUID() }, body: JSON.stringify({ expected_version: item.version, patch: { title: form.elements.__title.value, payload, sources: item.sources || [] }, notes: form.elements.__reason.value }) });
        await loadStageCatalog();
        await refresh();
        document.querySelector('#stageObjectDialog').close();
